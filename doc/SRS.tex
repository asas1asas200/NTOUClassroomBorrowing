...
\begin{tabularx}{\textwidth}{| c | X | X |}
	...
	0.2.3 & 新增管理方使用者介面分析         & 2022/11/03 \\ \hline
	0.3.0 & 修正至符合專案實作內容          & 2023/01/10 \\ \hline
	      &                      &            \\ \hline
\end{tabularx}
...
使用人性化的介面和即時的狀態顯示，除了讓使用者能避免到現場登記借用教室外，也能在借用教室時節省許多時間。
...
\begin{tabularx}{\textwidth}{|c|c|X|}
	...
	\color{blue}CBP-CBI-CR-1  & 教室編輯     & 管理員可以手動編輯教室資訊（包含新增、刪除教室）   \\ \hline
...
\begin{minipage}{0.6\linewidth}
	\includegraphics[height=0.4\textheight]{LogInGUI.jpg}
\end{minipage}
...
\end{minipage}
\begin{minipage}{0.4\linewidth}
	\includegraphics[height=0.4\textheight]{RegesterGUI.jpg}
\end{minipage}
...
\medskip
\begin{minipage}{0.6\linewidth}
	\includegraphics[height=0.4\textheight]{BorowingGUI.jpg}
\end{minipage}
\begin{minipage}{0.4\linewidth}
...
管理員可透過後台介面管理所有事務，旁邊標籤列用來分類管理功能，如下圖所示：

\bigskip

\includegraphics[width=0.9\textwidth]{Backstage.png}

\pagebreak

\subsubsection{帳號審核介面}

帳號審核頁面將會列出申請者基本資訊，以及是否通過 Email 驗證：

\bigskip

\centerline{\includegraphics[height=0.3\textheight]{BackstageAccount.png}}

\bigskip

按下使用者名稱後，將會以 Modal 的方式顯示該使用者的詳細資訊頁面，可以看到該使用者的詳細資料並進行審核：

\bigskip

\centerline{\includegraphics[height=0.3\textheight]{BackstageAccountDetail.png}}

\bigskip

\subsubsection{借用審核、鑰匙歸還介面}

與上述操作邏輯相似，僅列出之內容不同，故不再贅述。

\pagebreak

\subsubsection{教室編輯介面}

教室編輯頁面會顯示當前所有大樓、教室的基本資訊，並提供人性化介面的編輯、刪除、新增等功能：

\bigskip

\centerline{\includegraphics[height=0.3\textheight]{BackstageClassroom.png}}

\bigskip

對於每個教室，可以獨立編輯其資訊，並且預先設定好不可借用之時段：

\bigskip

\centerline{\includegraphics[height=0.3\textheight]{BackstageClassroomEdit.png}}
